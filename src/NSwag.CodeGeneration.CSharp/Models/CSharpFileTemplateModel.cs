...
namespace NSwag.CodeGeneration.CSharp.Models
{
    ...
    public class CSharpFileTemplateModel
    {
        ...
        private IEnumerable<string> JsonExceptionTypes => ResponsesInheritingFromException.Select(r =>
            _generator.GetTypeName(r.Value.GetActualResponseSchema(r.Key), r.Value.IsNullable(_settings.CSharpGeneratorSettings.SchemaType), "Response"));

        private IEnumerable<KeyValuePair<string, SwaggerResponse>> ResponsesInheritingFromException =>
            _document.Operations.SelectMany(o => o.Operation.ActualResponses.Where(r => r.Value.GetActualResponseSchema(r.Key)?.InheritsSchema(_resolver.ExceptionSchema) == true));
        ...
        public bool GenerateFileResponseClass =>
            ...
            _document.Operations.Any(o => o.Operation.ActualResponses.Any(r => r.Value.GetActualResponseSchema(r.Key)?.Type == JsonObjectType.File));
