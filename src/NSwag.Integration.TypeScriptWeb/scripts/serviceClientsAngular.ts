...
export class GeoClient extends MyBaseClass {
    ...
    protected processUploadFile(response: Response): Observable<boolean | null> {
        const status = response.status; 
    ...
    protected processSaveItems(response: Response): Observable<void> {
        ...
        if (status === 204) {
            ...
            let resultData450 = responseText === "" ? null : jsonParse(responseText, this.jsonParseReviver);
            result450 = resultData450 ? Exception.fromJS(resultData450, _mappings) : <any>null;
            return throwException("A server error occurred.", status, responseText, result450);
        } else if (status !== 200 && status !== 204) {
            const responseText = response.text();
            return throwException("An unexpected server error occurred.", status, responseText);
        }
    ...
    protected processGetUploadedFile(response: Response): Observable<Blob | null> {
        const status = response.status; 
...
export class PersonsClient extends MyBaseClass {
    ...
    protected processGet(response: Response): Observable<Person | null> {
        ...
        if (status === 200) {
            ...
            let resultData500 = responseText === "" ? null : jsonParse(responseText, this.jsonParseReviver);
