...
export class GeoClient {
    ...
    filter(currentStates: string[] | null): Promise<void> {
    ...
    reverse(values: string[] | null): Promise<string[] | null> {
    ...
    uploadFile(file: FileParameter | null): Promise<boolean | null> {
    ...
    protected processUploadFile(response: Response): Promise<boolean | null> {
        const status = response.status;
        let _headers: any = {}; if (response.headers && response.headers.forEach) { response.headers.forEach((v, k) => _headers[k] = v); };
        if (status === 200) {
            return response.text().then((_responseText) => {
    ...
    uploadFiles(files: FileParameter[] | null): Promise<void> {
    ...
    getUploadedFile(id: number, override: boolean): Promise<FileResponse | null> {
    ...
    postDouble(value: number | null): Promise<number | null> {
