...
export class Client {
    ...
    protected processProducts(response: Response): Promise<Product[] | null> {
    ...
    protected processPrice(response: Response): Promise<PriceEstimate[] | null> {
        const status = response.status;
        if (status === 200) {
            return response.text().then((_responseText) => {
     ...
     * @return An array of products
     */
    time(start_latitude: number, start_longitude: number, customer_uuid: string | undefined, product_id: string | undefined): Promise<Product[] | null> {
    ...
    protected processTime(response: Response): Promise<Product[] | null> {
        const status = response.status;
        if (status === 200) {
            return response.text().then((_responseText) => {
            let result200: Product[] | null = null;
    ...
    protected processMe(response: Response): Promise<Profile | null> {
        const status = response.status;
        if (status === 200) {
            return response.text().then((_responseText) => {
     ...
     */
    history(offset: number | undefined, limit: number | undefined): Promise<Activities | null> {
    ...
    protected processHistory(response: Response): Promise<Activities | null> {
        const status = response.status;
        if (status === 200) {
            return response.text().then((_responseText) => {
