namespace NSwag.SwaggerGeneration.WebApi.Versioned
{
    ...
    public class VersionedWebApiToSwaggerGenerator
    {
        ...
        public VersionedWebApiToSwaggerGenerator(VersionedApiExplorer explorer, VersionedWebApiToSwaggerGeneratorSettings settings)
            : this(explorer, settings, new SwaggerJsonSchemaGenerator(settings))
        {
        }

        public VersionedWebApiToSwaggerGenerator(VersionedApiExplorer explorer, VersionedWebApiToSwaggerGeneratorSettings settings, SwaggerJsonSchemaGenerator schemaGenerator)
        {
            Settings = settings;
            ...
            List<VersionedApiDescription> apiDescriptions, SwaggerGenerator swaggerGenerator, SwaggerSchemaResolver schemaResolver )
        {
            ...
            foreach ( var apiDescription in apiDescriptions )
            {
                ...
                if ( !path.StartsWith( "/", StringComparison.Ordinal ) )
                    path = "/" + path;
                if (!Enum.TryParse<SwaggerOperationMethod>(apiDescription.HttpMethod.Method, ignoreCase: true, result: out var swaggerOperationMethod))
                ...
                operations.Add(new Tuple<SwaggerOperationDescription, VersionedApiDescription>( operationDescription, apiDescription )  );
            }

            return await AddOperationDescriptionsToDocumentAsync(document, operations, swaggerGenerator, schemaResolver).ConfigureAwait( false );
        }

        private async Task<bool> AddOperationDescriptionsToDocumentAsync( SwaggerDocument document,  List<Tuple<SwaggerOperationDescription, VersionedApiDescription>> operations, SwaggerGenerator swaggerGenerator, SwaggerSchemaResolver schemaResolver)
        {
        ...
        private async Task<bool> RunOperationProcessorsAsync( SwaggerDocument document, VersionedApiDescription apiDescription,
